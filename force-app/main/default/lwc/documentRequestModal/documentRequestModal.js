...
export default class DocumentRequestModal extends NavigationMixin(LightningElement) {
  ...
  get isClaimDoc() {
    ...
    return (
      m === "cover letter - cms submission - carrier letter" ||
  ...
  pickTemplate = async (e) => {
    ...
    if (rule) {
      ...
      if (
        this.isCase &&
        (rule.match === "cover letter - cms submission - carrier letter" ||
          rule.match ===
        ...
        try {
          ...
          console.warn("Failed to fetch Supporting_Claims__c records:", err);
          this.claimRecords = [];
          this.selectedClaimIds = [];
        }
      }
      ...
      if (
        ...
        try {
          ...
          console.warn(
            "Failed to fetch Payee addresses for Submitter Letter or Diagnosis Context",
            err
          );
          const idx = this.currentInputs.findIndex(
            (i) => i.key === "addressToUse"
...
if (
  ...
  async generate() {
    ...
    if (
      this.isClaimDoc &&
      (!this.selectedClaimIds || this.selectedClaimIds.length === 0)
    ) {
      this.showToast(
        "No claims selected",
    ...
    try {
      ...
      if (this.isAdvancedDocument && this.currentRule && this.isCase) {
        ...
        try {
          ...
          if (isEdisonTemplate) {
            console.error(
              "🔥 EDISON DEBUG: CRITICAL - Edison template field population FAILED!"
            );
          }
          // Continue anyway
        }
        ...
        if (this.isSubmitterLetterDoc && this.isCase) {
          ...
          try {
            ...
            console.warn(
              "Failed to persist Submitter Letter address OR Diagnosis Codes",
              e
            );
            this.showToast(
              "Error",
              this.errMsg(e) ||
                "Failed to save the Address OR Diagnosis Codes on the Case.",
              "error"
            );
            this.loading = false;
            return;
        ...
        if (
          Array.isArray(this.currentInputs) &&
          this.currentInputs.length > 0
        ) {
        ...
        const lrPriorDisputeIssued = this.valuesByKey?.datePriorIssued || null;

        // Solo llamamos a Apex si hay al menos una fecha
        if (lrDateIssued || lrPriorDisputeIssued) {
            await setCaseLRDates({
            caseId: this.recordId,
            dateIssued: lrDateIssued || null,
            datePriorIssued: lrPriorDisputeIssued || null,
            });
        }
        } catch (e) {
        console.warn("Failed to persist LR dates", e);
        this.showToast(
            "Error",
            this.errMsg(e) || "Failed to save LR dates on the Case.",
            "error"
        );
        this.loading = false;
        return;
...
// ====== /DOWNLOAD-IMMEDIATE SUPPORT ======
  ...
  handleSelectIcd9 = (event) => {
    ...
    this.showToast(
      "Info",
      "ICD-9 selected. It will be saved when you click Generate.",
      "info"
    );
  };
  ...
  handleSelectIcd10 = (event) => {
    ...
    this.showToast(
      "Info",
      "ICD-10 selected. It will be saved when you click Generate.",
      "info"
    );
  };
}